...
## 📝 Scenario  
In certain scenarios, Logic App might trigger and reveal sensitive data. Leverage this information to obtain the flag.  
...
💡 *Hint: IDORs are common web application vulnerabilities. Consider different HTTP methods in a web request.*  
...
- Open the tool → *Open connect dialog* → Select **Blob container**.  
- Choose **Anonymously (my blob container allows public access)**.  
- Paste the container URL → Click **Next**.  
...
- Further enumeration of the `{name}` parameter reveals interesting behavior:  
  - Changing `{name}` to `test` → Creates a blob named **test** in the container.  
  - Changing `{name}` to `admin` → Creates a blob named **admin** containing the flag.
  

✅ **Flag:**  ashrujsm821ashmozuf821
