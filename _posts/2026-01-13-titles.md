...
- Open the tool → *Open connect dialog* → Select **Table**.  
- Choose **Shared access signature URL (SAS)**.  
- Paste the SAS URL → Click **Next**.  
...
- Further enumeration of the `{name}` parameter reveals interesting behavior:  
  - Changing `{name}` to `test` → Creates a new entry in the table with user `test`.  
  - Changing `{name}` to `admin` → Creates a new entry in the table containing the **flag**.  

✅ **Flag:**  : asmritbh830asmostln830 
...
- Here the Logic App is integrated with multiple other Azure services — in this case, a Storage Table.
...
- This is an example of **IDOR (Insecure Direct Object Reference)** — direct access to resources without proper checks.  

---

### 🔐 Why it needs to be secure  
